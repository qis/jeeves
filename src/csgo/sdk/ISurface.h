...
public:
  ...
  void DrawText(const wchar_t* format, Args&&... args) {
    if (sizeof...(Args) > 0) {
      std::array<wchar_t, 256> buffer;
      const auto size = std::swprintf(buffer.data(), buffer.size(), format, std::forward<Args>(args)...);
      if (size > 0) {
        DrawPrintText(buffer.data(), size);
        return;
      }
    }
    DrawPrintText(format, std::wcslen(format));
  }
};
